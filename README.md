...
the three services only ever talk to each other over HTTP, each with its own
Postgres. `screener` never touches price; `core` never re-derives fundamentals
the screener already computed; the existing valuation engine in `core` is
called in-process by the ranking layer, never re-implemented.
...
| [`core`](core/) | Java 25, Spring Boot 4, PostgreSQL | A standalone valuation-metrics engine (P/E, P/B, P/S, PEG, D/E, ROE, Graham Number, Lynch Fair Value) callable directly per request, *plus* an orchestration layer that pulls `screener`'s candidates, resolves and prices them via FMP, runs them through that same engine in-process, computes a price-dependent Value Score, and returns a combined ranked Top N. | [README](core/README.md) · [OpenAPI](core/docs/openapi.yaml) |
...
Each service can run standalone against its own Postgres, but the frontend's
Top Candidates view needs the full chain (`screener` → `core` → `frontend`)
to show real data. Suggested order:

1. **`screener`** — needs its own Postgres (`docker compose up -d`) and a
   `SEC_USER_AGENT_EMAIL` (SEC requires a real contact address in every
   request's User-Agent, or it returns 403). See [`screener/README.md`](screener/README.md).
2. **`core`** — needs its own Postgres, plus `SEC_USER_AGENT_EMAIL` (same
   requirement, its own SEC ticker-lookup client) and `FMP_API_KEY` (a free
   Financial Modeling Prep key is enough). See [`core/README.md`](core/README.md).
3. **`frontend`** — `npm start`, then open `http://localhost:4200`. See
   [`frontend/README.md`](frontend/README.md).

Neither Go nor Java has built-in `.env` file loading — both env vars need to
actually land in the process environment before each app starts (a real
`export`, an IDE run configuration, or a plugin like IntelliJ's EnvFile
reading a local `.env`), not just exist in a file sitting in the directory.

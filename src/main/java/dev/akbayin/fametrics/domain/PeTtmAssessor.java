...
public class PeTtmAssessor implements MetricAssessor<PeTtmRequest> {
    ...
    private static final BigDecimal FAIR_RANGE_LOWER = new BigDecimal("15");
    private static final BigDecimal FAIR_RANGE_UPPER = new BigDecimal("25");
    ...
    public MetricEvaluation evaluate(PeTtmRequest request, BigDecimal peTtm) {
        ...
        if (peTtm.compareTo(FAIR_RANGE_LOWER) < 0) {
            assessment = new Assessment(Rating.FAVORABLE, "Low P/E");
        } else if (peTtm.compareTo(FAIR_RANGE_UPPER) <= 0) {
    ...
    public String interpretation(PeTtmRequest request, BigDecimal peTtm, Assessment assessment) {
        return "At " + peTtm + "x trailing earnings, the stock's P/E is considered "
